...
const TAG_VIEW_STATE_ID = "tagview";

// Store current page globally within the panel's script scope
let panelCurrentPage = "";

/**
 * Initializes the Tag View's `SortableTree` instance using chevron SVG icons passed via config.
 * @param {TagPageTreeViewJsConfig} config - Configuration object for the tag view.
 * @returns {SortableTree} The created SortableTree instance.
 */
function createTagView(config) {
...
/**
 * Initializes the tag view panel, creates the SortableTree, and sets up action button listeners.
...
function initializeTagViewPanel(config) {
  panelCurrentPage = config.currentPage || "";

  config.dragAndDrop = { enabled: false };
  const tree = createTagView(config);
  ...
  const handleAction = (action) => {
    switch (action) {
      ...
      case "close-panel": {
        syscall("system.invokeFunction", "tagview.hide");
        return true;
      }
      case "refresh": {
        tree.clearState();
        syscall("system.invokeFunction", "tagview.show");
        return true;
      }
  ...
  document.querySelectorAll("[data-tagview-action]").forEach((el) => {
    const action = el.dataset["tagviewAction"];
  ...
  console.log("Tag View panel initialized. Current page:", panelCurrentPage);
}
